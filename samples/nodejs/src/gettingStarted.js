...
async function run() {
  ...
  console.log("poolGenesisTxnPath : ", poolGenesisTxnPath);
  let poolConfig = {
  ...
  try {
    const wallet = await indy.createWallet(
      stewardWalletConfig,
  ...
  let stewardWallet = await indy.openWallet(
    stewardWalletConfig,
    stewardWalletCredentials
  );
  ...
  let [stewardDid] = await indy.createAndStoreMyDid(
    stewardWallet,
    stewardDidInfo
  );
  ...
  let thriftDid = await getVerinym(
    poolHandle,
    "Sovrin Steward",
    stewardWallet,
    stewardDid,
  ...
  ] = await indy.issuerCreateSchema(governmentDid, "Job-Certificate", "0.2", [
    "first_name",
    "last_name",
    "salary",
    "employee_status",
  ...
  let [
    transcriptSchemaId,
    transcriptSchema,
  ] = await indy.issuerCreateSchema(governmentDid, "Transcript", "1.2", [
    "first_name",
    "last_name",
    "degree",
    "status",
  ...
  [, transcriptSchema] = await getSchema(
    poolHandle,
    faberDid,
    transcriptSchemaId
  );
  ...
  console.log("faberTranscriptCredDefJson : ", faberTranscriptCredDefJson);
  ...
  [, jobCertificateSchema] = await getSchema(
    poolHandle,
    acmeDid,
    jobCertificateSchemaId
  );
  ...
  let [
    acmeJobCertificateCredDefId,
    acmeJobCertificateCredDefJson,
  ] = await indy.issuerCreateAndStoreCredentialDef(
    acmeWallet,
    acmeDid,
    jobCertificateSchema,
  ...
  console.log("transcriptCredOfferJson", transcriptCredOfferJson);
  ...
  let authcryptedJobApplicationProofRequestJson = await indy.cryptoAuthCrypt(
    ...
    acmeAliceKey,
    aliceAcmeVerkey,
    Buffer.from(JSON.stringify(jobApplicationProofRequestJson), "utf8")
  );
  ...
  let authcryptedJobApplicationProofJson = await indy.cryptoAuthCrypt(
    aliceWallet,
    aliceAcmeKey,
    acmeAliceVerkey,
    Buffer.from(JSON.stringify(jobApplicationProofJson), "utf8")
  );
  ...
  [
    ,
    decryptedJobApplicationProofJson,
    decryptedJobApplicationProof,
  ] = await authDecrypt(
    acmeWallet,
    acmeAliceKey,
    authcryptedJobApplicationProofJson
  );
  ...
  [
    ...
    revocRefDefsJson,
    revocRegsJson,
  ] = await verifierGetEntitiesFromLedger(
    poolHandle,
    acmeDid,
    decryptedJobApplicationProof["identifiers"],
    "Acme"
  );
  ...
  assert(
    "Bachelor of Science, Marketing" ===
      decryptedJobApplicationProof["requested_proof"]["revealed_attrs"][
        "attr3_referent"
      ]["raw"]
  );
  assert(
  ...
  assert(
    "Alice" ===
      decryptedJobApplicationProof["requested_proof"]["self_attested_attrs"][
        "attr1_referent"
      ]
  );
  assert(
  ...
  const a = await indy.verifierVerifyProof(
  ...
  console.log('"Acme" -> Get key for Alice did');
  aliceAcmeVerkey = await indy.keyForDid(
    poolHandle,
    acmeWallet,
    acmeAliceConnectionResponse["did"]
  );
  ...
  let authdecryptedJobCertificateCredOfferJson,
  ...
  let aliceThriftVerkey = await indy.keyForDid(
    poolHandle,
    thriftWallet,
    thriftAliceConnectionResponse["did"]
  );

  console.log(
    '"Thrift" -> Authcrypt "Loan-Application-Basic" Proof Request for Alice'
  );
  let authcryptedApplyLoanProofRequestJson = await indy.cryptoAuthCrypt(
    thriftWallet,
    thriftAliceKey,
    aliceThriftVerkey,
    Buffer.from(JSON.stringify(applyLoanProofRequestJson), "utf8")
  );
  ...
  let [
    thriftAliceVerkey,
    authdecryptedApplyLoanProofRequestJson,
  ] = await authDecrypt(
    aliceWallet,
    aliceThriftKey,
    authcryptedApplyLoanProofRequestJson
  );
  ...
  let searchForJApplyLoanProofRequest = await indy.proverSearchCredentialsForProofReq(
    aliceWallet,
    authdecryptedApplyLoanProofRequestJson,
    null
  );

  credentials = await indy.proverFetchCredentialsForProofReq(
    searchForJApplyLoanProofRequest,
    "attr1_referent",
    100
  );
  ...
  credsForApplyLoanProof[
    `${credForPredicate1["referent"]}`
  ] = credForPredicate1;
  credsForApplyLoanProof[
    `${credForPredicate2["referent"]}`
  ] = credForPredicate2;
  ...
  let aliceApplyLoanProofJson = await indy.proverCreateProof(
    aliceWallet,
    authdecryptedApplyLoanProofRequestJson,
    applyLoanRequestedCredsJson,
    aliceMasterSecretId,
    schemasJson,
    credDefsJson,
    revocStatesJson
  ...
  let authcryptedAliceApplyLoanProofJson = await indy.cryptoAuthCrypt(
    aliceWallet,
    aliceThriftKey,
    thriftAliceVerkey,
    Buffer.from(JSON.stringify(aliceApplyLoanProofJson), "utf8")
  );
  ...
  [
    ,
    authdecryptedAliceApplyLoanProofJson,
    authdecryptedAliceApplyLoanProof,
  ] = await authDecrypt(
    thriftWallet,
    thriftAliceKey,
    authcryptedAliceApplyLoanProofJson
  );
  ...
  [
    ...
    poolHandle,
    thriftDid,
    authdecryptedAliceApplyLoanProof["identifiers"],
    "Thrift"
  );
  ...
  assert(
    "Permanent" ===
      authdecryptedAliceApplyLoanProof["requested_proof"]["revealed_attrs"][
        "attr1_referent"
      ]["raw"]
  );

  assert(
    await indy.verifierVerifyProof(
      applyLoanProofRequestJson,
      authdecryptedAliceApplyLoanProofJson,
      schemasJson,
      credDefsJson,
      revocDefsJson,
  ...
  console.log('"Thrift" -> Get key for Alice did');
  aliceThriftVerkey = await indy.keyForDid(
    poolHandle,
    thriftWallet,
    thriftAliceConnectionResponse["did"]
  );
  ...
  let authcryptedApplyLoanKycProofRequestJson = await indy.cryptoAuthCrypt(
    thriftWallet,
    thriftAliceKey,
    aliceThriftVerkey,
    Buffer.from(JSON.stringify(applyLoanKycProofRequestJson), "utf8")
  );
  ...
  [
    thriftAliceVerkey,
    authdecryptedApplyLoanKycProofRequestJson,
  ] = await authDecrypt(
    aliceWallet,
    aliceThriftKey,
    authcryptedApplyLoanKycProofRequestJson
  );
  ...
  let searchForApplyLoanKycProofRequest = await indy.proverSearchCredentialsForProofReq(
    aliceWallet,
    authdecryptedApplyLoanKycProofRequestJson,
    null
  );
  ...
  credentials = await indy.proverFetchCredentialsForProofReq(
    searchForApplyLoanKycProofRequest,
    "attr1_referent",
    100
  );
  credForAttr1 = credentials[0]["cred_info"];
  ...
  let aliceApplyLoanKycProofJson = await indy.proverCreateProof(
    aliceWallet,
    authdecryptedApplyLoanKycProofRequestJson,
    applyLoanKycRequestedCredsJson,
    aliceMasterSecretId,
    schemasJson,
    credDefsJson,
    revocStatesJson
  ...
  let authcryptedAliceApplyLoanKycProofJson = await indy.cryptoAuthCrypt(
    aliceWallet,
    aliceThriftKey,
    thriftAliceVerkey,
    Buffer.from(JSON.stringify(aliceApplyLoanKycProofJson), "utf8")
  );
  ...
  [, , authdecryptedAliceApplyLoanKycProof] = await authDecrypt(
    thriftWallet,
    thriftAliceKey,
    authcryptedAliceApplyLoanKycProofJson
  );
  ...
  [
    ...
    poolHandle,
    thriftDid,
    authdecryptedAliceApplyLoanKycProof["identifiers"],
    "Thrift"
  );
  ...
  assert(
    "Alice" ===
      authdecryptedAliceApplyLoanKycProof["requested_proof"]["revealed_attrs"][
        "attr1_referent"
      ]["raw"]
  );
  assert(
  ...
  assert(
    await indy.verifierVerifyProof(
      applyLoanKycProofRequestJson,
      authdecryptedAliceApplyLoanKycProof,
      schemasJson,
      credDefsJson,
      revocDefsJson,
      revocRegsJson
...
async function proverGetEntitiesFromLedger(
  ...
  for (let referent of Object.keys(identifiers)) {
    ...
    // console.log("poolHandle : ", poolHandle, "did : ", did, "item : ", item);
